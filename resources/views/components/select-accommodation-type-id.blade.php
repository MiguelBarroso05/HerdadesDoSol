<select x-data
        x-init="
            ...
            $el.addEventListener('change', () => {
                @this.set('selectedAccommodationTypeId', $el.value);
                @this.call('show_accommodations');
            });
        " wire:model="{{ $attributes->get('wire:model') }}" wire:change="{{ $attributes->get('wire:change') }}"  data-hs-select='{
  ...
  "dropdownVerticalFixedPlacement": "{{ $fixed }}",
  ...
  "extraMarkup": "<div class=\"absolute top-1/2 end-3 -translate-y-1/2\"><svg class=\"shrink-0 size-3.5 text-gray-500 dark:text-neutral-500 \" xmlns=\"http://www.w3.org/2000/svg\" width=\"24\" height=\"24\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"m7 15 5 5 5-5\"/><path d=\"m7 9 5-5 5 5\"/></svg></div>"
    ...
    <option {{ $object ? 'disabled' : '' }}> {{ $placeholder }}</option>
    @if ($object)
        @foreach ($object as $o)
            <option value="{{ $o->id }}" {{ $paramter == $o->id ? 'selected' : '' }}>{{ $o->$optionText }}
            </option>
        @endforeach
    @endif
</select>
