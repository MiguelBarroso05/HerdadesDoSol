    ...
    <form wire:submit="submit" method="POST"
        ...
        <div class="hs-w-80 hs-bg-card hs-p-5 hs-rounded-3 hs-d-flex hs-flex-row hs-justify-content-between">
            <div class="scroll-container hs-w-70 hs-pe-2" style="max-height: 580px; overflow-y: scroll;">
                ...
                @else
                    <div class="hs-d-flex hs-flex-column">
                        ...
                        <a href="{{ route('payment-methods') }}" class="hs-btn hs-btn-primary hs-col-4">Please add a
                            billing
                            information</a>
                    </div>
                    <hr>
                ...
                @if (!$isReservation)
                    <div class="hs-d-flex hs-flex-column">
                        ...
                        @else
                            ...
                            <a href="{{ route('personal-info') }}" class="hs-btn hs-btn-primary hs-col-4">Please add a
                                address</a>
                        @endif
                    </div>
                @endif
            ...
            <div class=" hs-w-25">
                        ...
                        <div class="hs-d-flex hs-flex-row hs-justify-content-between ">
                            <div class="hs-py-2">
                                ...
                                <div class="hs-d-flex hs-flex-row hs-align-items-center">
                                    ...
                                    <span class="hs-d-flex hs-w-50"><livewire:NumberInput
                                            wire:key="quantity{{$loop->index}}" :value="$product['quantity']"
                                            class="form-control " :max="8" :name="$id"></span>
                                </div>
                            </div>
                            <div class="hs-py-1">
                ...
                @elseif ($isReservation)
                    ...
                    <div class="hs-d-flex hs-justify-content-between">
                        <h5>Reservation</h5>
                        <a class="hover:cursor-pointer" wire:click="cancelReservation"><i
                                class="bi bi-x text-red-600 text-2xl"></i></a>
                    </div>
                    <p>
                        You have a reservation at <span class="hs-fw-bold">{{ $reservation->estate->name }}</span> from
                        <span class="hs-fw-bold">{{ $reservation->entry_date }}</span> to <span
                            class="hs-fw-bold">{{ $reservation->exit_date }}</span> in a
                        <span class="hs-fw-bold">{{ $reservation->accommodation->accommodationType->name }}</span>
                        accommodation with a size of
                        <span class="hs-fw-bold">{{ $reservation->accommodation->size }}</span> people.
                    </p>
                    @if ( isset($reservationActivities) && count($reservationActivities) > 0)

                        <div class="max-h-[190px] overflow-y-hidden ">
                        <p class="m-0">Activities:</p>
                        <ul>
                            @foreach ($reservationActivities as $activity)
                                <li class="font-bold text-black">{{ $activity['name'] }}</li>
                            @endforeach
                        </ul>
                        </div>
                    @endif
            ...
            .substring(0, 3)" placeholder="xxx" maxlength="3" required>
                ...
                <div>
                    <button type="submit" class="hs-btn hs-btn-primary hs-w-100">Buy</button>
                </div>
