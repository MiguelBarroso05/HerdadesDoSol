<div class="relative w-full max-w-md" x-data="{ open: false }">
    <!-- Dropdown Trigger -->
    <div class="relative py-3 px-4 w-full cursor-pointer bg-white border border-gray-200 rounded-lg text-start text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400"
        @click="open = !open">
        <div class="flex flex-wrap gap-2">
            @forelse($selected as $id)
                <span class="bg-blue-100 text-blue-600 px-2 py-1 rounded-full text-sm">
                    {{ $activityMap[$id] ?? 'Unknown' }}
                </span>
            @empty
        ...
        @foreach ($options as $group)
            <div class="py-2 px-4 border-b border-gray-200">
                ...
                @foreach ($group['activities'] as $activity)
                    <label
                        class="flex items-center px-4 py-2 hover:bg-gray-100 dark:hover:bg-neutral-800 cursor-pointer">
                        <input type="checkbox" wire:click="toggleSelection({{ $activity['id'] }})"
                            {{ in_array($activity['id'], $selected) ? 'checked' : '' }}
                            class="form-checkbox rounded text-blue-500 hidden" />
