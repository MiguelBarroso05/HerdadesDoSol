<div class="hs-d-flex ">
    ...
    <div class="hs-bg-gradient-danger hs-w-70">
        <form wire:submit="submit">
            <div class="hs-bg-dark hs-w-100 hs-py-3 hs-d-inline-flex hs-justify-content-between hs-px-1">
                ...
                <!-- End Select -->
                <div class="hs-w-25">
                    <x-datetime-picker wire.model="entryDate" value="{{ $entryDate }}" id="entry_date"
                        wire:model.live="model6" placeholder="Appointment Date" parse-format="DD-MM-YYYY" />
                </div>
                <div class="hs-w-25">
                    <x-datetime-picker wire.model="exitDate" value="{{ $exitDate }}" id="departure_date"
                        wire:model.live="model2" placeholder="Appointment Date" parse-format="DD-MM-YYYY" />
                </div>
            </div>
    ...
    <div class="hs-bg-gradient-warning hs-w-30 hs-p-3">
        <div class="hs-d-inline-flex">
            <div class="hs-w-45">
                <x-number wire:model="$groupsize" label="Group size" value="{{ $groupsize }}" />
            </div>
            <div class="hs-w-45">
                <x-number wire:model="$children" label="Children" value="{{ $children }}" />
            </div>
        </div>
        <div class="relative">
            ...
            <x-dropdown-input wire:model="selectedAccommodation" :multiple="false" :placeholder="'Select a Accommodation'" :fixed="'bottom'" :name="'accommodation'"
                :object="$accommodations" :user="auth()->user()" :paramter="null" :optionText="'size'" />
        </div>

        <x-dropdown-input :multiple="true" :placeholder="'Add Activities to reserve'" :fixed="'bottom'" :name="'activities'" :object="$estates"
            :user="auth()->user()" :paramter="null" :optionText="'name'" />
    ...
    @push('js')
        <script>
            ...
            function updateCalendar() {
                ...
                const remainingDays = 35 - (firstDayOfMonth +
                    daysInMonth); // Total de dias no calendário (5 linhas * 7 colunas)
                for (let i = 1; i <= remainingDays; i++) {
                    ...
                    button.dataset.year = currentMonth + 1 > 11 ? currentYear + 1 :
                        currentYear; // Ano seguinte, se for Dezembro
                    calendar.appendChild(button);
                }
